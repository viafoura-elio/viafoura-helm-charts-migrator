...
metadata:
  annotations:
    ...
    {{- include "auth-service.datadogAnnotations" . | indent 4 }}
    {{- end }}
    {{- if .Values.istio.enabled }}
    {{- $sidecarContent := include "auth-service.sidecarAnnotations" . | trim }}
    {{- if $sidecarContent }}
...
spec:
  ...
  containers:
    - name: {{ .Chart.Name }}
      ...
      imagePullPolicy: {{ include "auth-service.imagePullPolicy" . }}
      ports:
        {{- include "auth-service.containerPorts" . | nindent 8 }}
      ...
      envFrom:
        ...
        - configMapRef:
            name: {{ include "auth-service.fullname" . }}-configMapEnvVars
        {{- end }}
        {{- if .Values.secretsEnvVars }}
        - secretRef:
            name: {{ include "auth-service.fullname" . }}-secretsEnvVars
        {{- end }}
      {{- end }}
      volumeMounts:
        - name: dsdsocket
        ...
        - name: {{ include "auth-service.fullname" . }}-config-volume
  ...
  {{- include "auth-service.podAffinity" . | nindent 2 }}
  {{- end }}
  ...
  {{- include "auth-service.topologySpreadConstraints" . | nindent 2 }}
