...
*/}}
{{- define "auth-service.datadogAnnotations" -}}
{{- if .Values.datadog.enabled -}}
{{- include "auth-service.datadogChecks" . }}
{{- include "auth-service.datadogLogs" . }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "auth-service.datadogChecks" -}}
{{- $instanceConfig := include "auth-service.datadogInstanceConfig" . }}
{{- if $instanceConfig }}
...
ad.datadoghq.com/{{ include "auth-service.datadogContainerName" . }}.checks: |
  {{ $config | toJson }}
{{- end }}
{{- end -}}
...
*/}}
{{- define "auth-service.datadogInstanceConfig" -}}
{{- if eq (.Values.datadog.integration | default "openmetrics") "openmetrics" -}}
{{- include "auth-service.datadogOpenmetricsConfig" . }}
{{- else if eq .Values.datadog.integration "jmx" -}}
{{- include "auth-service.datadogJmxConfig" . }}
{{- else -}}
{{- include "auth-service.datadogCustomConfig" . }}
{{- end -}}
{{- include "auth-service.datadogCommonInstanceConfig" . }}
{{- end -}}

{{/*
OpenMetrics integration configuration
*/}}
{{- define "auth-service.datadogOpenmetricsConfig" -}}
...
*/}}
{{- define "auth-service.datadogJmxConfig" -}}
host: "%%host%%"
...
*/}}
{{- define "auth-service.datadogCustomConfig" -}}
...
*/}}
{{- define "auth-service.datadogCommonInstanceConfig" -}}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    ...
    {{- if eq (len $parts) 2 }}
      ...
      {{- $value := index $parts 1 }}
      {{- if eq $key "version" }}
        {{- $_ := set $tagsDict $key (include "auth-service.appVersion" $root) }}
      {{- else }}
...
{{- end }}
  ...
  {{- if not (hasKey $tagsDict "version") }}
    {{- $_ := set $tagsDict "version" (include "auth-service.appVersion" $root) }}
  {{- end }}
  {{- /* Output unique tags */}}
...
*/}}
{{- define "auth-service.datadogLogs" -}}
{{- if .Values.datadog.logs.enabled }}
ad.datadoghq.com/{{ include "auth-service.datadogContainerName" . }}.logs: |
...
*/}}
{{- define "auth-service.datadogLogsConfig" -}}
...
*/}}
{{- define "auth-service.datadogLabels" -}}
{{- if .Values.datadog.enabled }}
datadog.monitoring: "true"
datadog.service: {{ include "auth-service.datadogContainerName" . | quote }}
...
*/}}
{{- define "auth-service.datadogTags" -}}
{{- $tags := list }}
{{- $root := . }}
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    {{- if hasPrefix "version:" . }}
      {{- $tags = append $tags (printf "version:%s" (include "auth-service.appVersion" $root)) }}
    {{- else }}
      {{- $tags = append $tags . }}
    {{- end }}
  {{- end }}
...
*/}}
{{- define "auth-service.datadogEnvVars" -}}
...
- name: DD_VERSION
  value: {{ include "auth-service.appVersion" . | quote }}
...
*/}}
{{- define "auth-service.datadogContainerName" -}}
