...
*/}}
{{- define "livechat.datadogAnnotations" -}}
{{- if .Values.datadog.enabled -}}
{{- include "livechat.datadogChecks" . }}
{{- include "livechat.datadogLogs" . }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "livechat.datadogChecks" -}}
{{- $instanceConfig := include "livechat.datadogInstanceConfig" . }}
{{- if $instanceConfig }}
...
ad.datadoghq.com/{{ include "livechat.datadogContainerName" . }}.checks: |
  {{ $config | toJson }}
{{- end }}
{{- end -}}
...
*/}}
{{- define "livechat.datadogInstanceConfig" -}}
{{- if eq (.Values.datadog.integration | default "openmetrics") "openmetrics" -}}
{{- include "livechat.datadogOpenmetricsConfig" . }}
{{- else if eq .Values.datadog.integration "jmx" -}}
{{- include "livechat.datadogJmxConfig" . }}
{{- else -}}
{{- include "livechat.datadogCustomConfig" . }}
{{- end -}}
{{- include "livechat.datadogCommonInstanceConfig" . }}
{{- end -}}

{{/*
OpenMetrics integration configuration
*/}}
{{- define "livechat.datadogOpenmetricsConfig" -}}
...
*/}}
{{- define "livechat.datadogJmxConfig" -}}
host: "%%host%%"
...
*/}}
{{- define "livechat.datadogCustomConfig" -}}
...
*/}}
{{- define "livechat.datadogCommonInstanceConfig" -}}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    ...
    {{- if eq (len $parts) 2 }}
      ...
      {{- $value := index $parts 1 }}
      {{- if eq $key "version" }}
        {{- $_ := set $tagsDict $key (include "livechat.appVersion" $root) }}
      {{- else }}
...
{{- end }}
  ...
  {{- if not (hasKey $tagsDict "version") }}
    {{- $_ := set $tagsDict "version" (include "livechat.appVersion" $root) }}
  {{- end }}
  {{- /* Output unique tags */}}
...
*/}}
{{- define "livechat.datadogLogs" -}}
{{- if .Values.datadog.logs.enabled }}
ad.datadoghq.com/{{ include "livechat.datadogContainerName" . }}.logs: |
...
*/}}
{{- define "livechat.datadogLogsConfig" -}}
...
*/}}
{{- define "livechat.datadogLabels" -}}
{{- if .Values.datadog.enabled }}
datadog.monitoring: "true"
datadog.service: {{ include "livechat.datadogContainerName" . | quote }}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    {{- if hasPrefix "version:" . }}
      {{- $tags = append $tags (printf "version:%s" (include "livechat.appVersion" $root)) }}
    {{- else }}
      {{- $tags = append $tags . }}
    {{- end }}
  {{- end }}
...
*/}}
{{- define "livechat.datadogEnvVars" -}}
...
- name: DD_VERSION
  value: {{ include "livechat.appVersion" . | quote }}
...
*/}}
{{- define "livechat.datadogContainerName" -}}
