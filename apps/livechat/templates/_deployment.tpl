...
metadata:
  annotations:
    ...
    {{- include "livechat.datadogAnnotations" . | indent 4 }}
    {{- end }}
    {{- if .Values.istio.enabled }}
    {{- $sidecarContent := include "livechat.sidecarAnnotations" . | trim }}
    {{- if $sidecarContent }}
...
spec:
  ...
  containers:
    - name: {{ .Chart.Name }}
      ...
      image: {{ include "livechat.image" . }}
      imagePullPolicy: {{ include "livechat.imagePullPolicy" . }}
      ports:
        {{- include "livechat.containerPorts" . | nindent 8 }}
      ...
      envFrom:
        ...
        - configMapRef:
            name: {{ include "livechat.fullname" . }}-configMapEnvVars
        {{- end }}
        {{- if .Values.secretsEnvVars }}
        - secretRef:
            name: {{ include "livechat.fullname" . }}-secretsEnvVars
        {{- end }}
      {{- end }}
      volumeMounts:
        - name: dsdsocket
        ...
        - name: {{ include "livechat.fullname" . }}-config-volume
  ...
  {{- include "livechat.podAffinity" . | nindent 2 }}
  {{- end }}
  ...
  {{- include "livechat.topologySpreadConstraints" . | nindent 2 }}
  {{- end }}
  volumes:
