...
*/}}
{{- define "tyrion.datadogAnnotations" -}}
{{- if .Values.datadog.enabled -}}
{{- include "tyrion.datadogChecks" . }}
{{- include "tyrion.datadogLogs" . }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "tyrion.datadogChecks" -}}
{{- $instanceConfig := include "tyrion.datadogInstanceConfig" . }}
{{- if $instanceConfig }}
...
ad.datadoghq.com/{{ include "tyrion.datadogContainerName" . }}.checks: |
  {{ $config | toJson }}
{{- end }}
{{- end -}}
...
*/}}
{{- define "tyrion.datadogInstanceConfig" -}}
{{- if eq (.Values.datadog.integration | default "openmetrics") "openmetrics" -}}
{{- include "tyrion.datadogOpenmetricsConfig" . }}
{{- else if eq .Values.datadog.integration "jmx" -}}
{{- include "tyrion.datadogJmxConfig" . }}
{{- else -}}
{{- include "tyrion.datadogCustomConfig" . }}
{{- end -}}
{{- include "tyrion.datadogCommonInstanceConfig" . }}
{{- end -}}

{{/*
OpenMetrics integration configuration
*/}}
{{- define "tyrion.datadogOpenmetricsConfig" -}}
...
*/}}
{{- define "tyrion.datadogJmxConfig" -}}
host: "%%host%%"
...
*/}}
{{- define "tyrion.datadogCustomConfig" -}}
...
*/}}
{{- define "tyrion.datadogCommonInstanceConfig" -}}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    ...
    {{- if eq (len $parts) 2 }}
      ...
      {{- $value := index $parts 1 }}
      {{- if eq $key "version" }}
        {{- $_ := set $tagsDict $key (include "tyrion.appVersion" $root) }}
      {{- else }}
...
{{- end }}
  ...
  {{- if not (hasKey $tagsDict "version") }}
    {{- $_ := set $tagsDict "version" (include "tyrion.appVersion" $root) }}
  {{- end }}
  {{- /* Output unique tags */}}
...
*/}}
{{- define "tyrion.datadogLogs" -}}
{{- if .Values.datadog.logs.enabled }}
ad.datadoghq.com/{{ include "tyrion.datadogContainerName" . }}.logs: |
...
*/}}
{{- define "tyrion.datadogLogsConfig" -}}
...
*/}}
{{- define "tyrion.datadogLabels" -}}
{{- if .Values.datadog.enabled }}
datadog.monitoring: "true"
datadog.service: {{ include "tyrion.datadogContainerName" . | quote }}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    {{- if hasPrefix "version:" . }}
      {{- $tags = append $tags (printf "version:%s" (include "tyrion.appVersion" $root)) }}
    {{- else }}
      {{- $tags = append $tags . }}
    {{- end }}
  {{- end }}
...
- name: DD_VERSION
  value: {{ include "tyrion.appVersion" . | quote }}
...
*/}}
{{- define "tyrion.datadogContainerName" -}}
