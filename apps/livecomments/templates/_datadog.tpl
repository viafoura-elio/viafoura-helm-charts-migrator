...
*/}}
{{- define "livecomments.datadogAnnotations" -}}
{{- if .Values.datadog.enabled -}}
{{- include "livecomments.datadogChecks" . }}
{{- include "livecomments.datadogLogs" . }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "livecomments.datadogChecks" -}}
{{- $instanceConfig := include "livecomments.datadogInstanceConfig" . }}
{{- if $instanceConfig }}
...
ad.datadoghq.com/{{ include "livecomments.datadogContainerName" . }}.checks: |
  {{ $config | toJson }}
{{- end }}
{{- end -}}
...
*/}}
{{- define "livecomments.datadogInstanceConfig" -}}
{{- if eq (.Values.datadog.integration | default "openmetrics") "openmetrics" -}}
{{- include "livecomments.datadogOpenmetricsConfig" . }}
{{- else if eq .Values.datadog.integration "jmx" -}}
{{- include "livecomments.datadogJmxConfig" . }}
{{- else -}}
{{- include "livecomments.datadogCustomConfig" . }}
{{- end -}}
{{- include "livecomments.datadogCommonInstanceConfig" . }}
{{- end -}}

{{/*
OpenMetrics integration configuration
*/}}
{{- define "livecomments.datadogOpenmetricsConfig" -}}
...
*/}}
{{- define "livecomments.datadogJmxConfig" -}}
host: "%%host%%"
...
*/}}
{{- define "livecomments.datadogCustomConfig" -}}
...
*/}}
{{- define "livecomments.datadogCommonInstanceConfig" -}}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    ...
    {{- if eq (len $parts) 2 }}
      ...
      {{- $value := index $parts 1 }}
      {{- if eq $key "version" }}
        {{- $_ := set $tagsDict $key (include "livecomments.appVersion" $root) }}
      {{- else }}
...
{{- end }}
  ...
  {{- if not (hasKey $tagsDict "version") }}
    {{- $_ := set $tagsDict "version" (include "livecomments.appVersion" $root) }}
  {{- end }}
  {{- /* Output unique tags */}}
...
*/}}
{{- define "livecomments.datadogLogs" -}}
{{- if .Values.datadog.logs.enabled }}
ad.datadoghq.com/{{ include "livecomments.datadogContainerName" . }}.logs: |
...
*/}}
{{- define "livecomments.datadogLogsConfig" -}}
...
*/}}
{{- define "livecomments.datadogLabels" -}}
{{- if .Values.datadog.enabled }}
datadog.monitoring: "true"
datadog.service: {{ include "livecomments.datadogContainerName" . | quote }}
...
*/}}
{{- define "livecomments.datadogTags" -}}
{{- $tags := list }}
{{- $root := . }}
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    {{- if hasPrefix "version:" . }}
      {{- $tags = append $tags (printf "version:%s" (include "livecomments.appVersion" $root)) }}
    {{- else }}
      {{- $tags = append $tags . }}
    {{- end }}
  {{- end }}
...
*/}}
{{- define "livecomments.datadogEnvVars" -}}
...
- name: DD_VERSION
  value: {{ include "livecomments.appVersion" . | quote }}
...
*/}}
{{- define "livecomments.datadogContainerName" -}}
