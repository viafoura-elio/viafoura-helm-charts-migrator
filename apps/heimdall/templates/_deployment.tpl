...
metadata:
  annotations:
    ...
    {{- include "heimdall.datadogAnnotations" . | indent 4 }}
    {{- end }}
    {{- if .Values.istio.enabled }}
    {{- $sidecarContent := include "heimdall.sidecarAnnotations" . | trim }}
    {{- if $sidecarContent }}
...
spec:
  ...
  containers:
    - name: {{ .Chart.Name }}
      securityContext:
        ...
        {{- end }}
      image: {{ include "heimdall.image" . }}
      imagePullPolicy: {{ include "heimdall.imagePullPolicy" . }}
      ports:
        {{- include "heimdall.containerPorts" . | nindent 8 }}
      ...
      envFrom:
        ...
        - configMapRef:
            name: {{ include "heimdall.fullname" . }}-configMapEnvVars
        {{- end }}
        {{- if .Values.secretsEnvVars }}
        - secretRef:
            name: {{ include "heimdall.fullname" . }}-secretsEnvVars
        {{- end }}
      {{- end }}
      volumeMounts:
        - name: dsdsocket
        ...
        - name: {{ include "heimdall.fullname" . }}-config-volume
  ...
  {{- include "heimdall.podAffinity" . | nindent 2 }}
  {{- end }}
  ...
  {{- include "heimdall.topologySpreadConstraints" . | nindent 2 }}
