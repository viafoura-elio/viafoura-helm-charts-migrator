...
*/}}
{{- define "heimdall.datadogAnnotations" -}}
{{- if .Values.datadog.enabled -}}
{{- include "heimdall.datadogChecks" . }}
{{- include "heimdall.datadogLogs" . }}
{{- end -}}
{{- end -}}
...
*/}}
{{- define "heimdall.datadogChecks" -}}
{{- $instanceConfig := include "heimdall.datadogInstanceConfig" . }}
{{- if $instanceConfig }}
...
ad.datadoghq.com/{{ include "heimdall.datadogContainerName" . }}.checks: |
  {{ $config | toJson }}
{{- end }}
{{- end -}}
...
*/}}
{{- define "heimdall.datadogInstanceConfig" -}}
{{- if eq (.Values.datadog.integration | default "openmetrics") "openmetrics" -}}
{{- include "heimdall.datadogOpenmetricsConfig" . }}
{{- else if eq .Values.datadog.integration "jmx" -}}
{{- include "heimdall.datadogJmxConfig" . }}
{{- else -}}
{{- include "heimdall.datadogCustomConfig" . }}
{{- end -}}
{{- include "heimdall.datadogCommonInstanceConfig" . }}
{{- end -}}

{{/*
OpenMetrics integration configuration
*/}}
{{- define "heimdall.datadogOpenmetricsConfig" -}}
...
*/}}
{{- define "heimdall.datadogJmxConfig" -}}
host: "%%host%%"
...
*/}}
{{- define "heimdall.datadogCustomConfig" -}}
...
*/}}
{{- define "heimdall.datadogCommonInstanceConfig" -}}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    ...
    {{- if eq (len $parts) 2 }}
      ...
      {{- $value := index $parts 1 }}
      {{- if eq $key "version" }}
        {{- $_ := set $tagsDict $key (include "heimdall.appVersion" $root) }}
      {{- else }}
...
{{- end }}
  ...
  {{- if not (hasKey $tagsDict "version") }}
    {{- $_ := set $tagsDict "version" (include "heimdall.appVersion" $root) }}
  {{- end }}
  {{- /* Output unique tags */}}
...
*/}}
{{- define "heimdall.datadogLogs" -}}
{{- if .Values.datadog.logs.enabled }}
ad.datadoghq.com/{{ include "heimdall.datadogContainerName" . }}.logs: |
...
*/}}
{{- define "heimdall.datadogLogsConfig" -}}
...
*/}}
{{- define "heimdall.datadogLabels" -}}
{{- if .Values.datadog.enabled }}
datadog.monitoring: "true"
datadog.service: {{ include "heimdall.datadogContainerName" . | quote }}
...
{{- if kindIs "slice" .Values.datadog.tags }}
  ...
  {{- range .Values.datadog.tags }}
    {{- if hasPrefix "version:" . }}
      {{- $tags = append $tags (printf "version:%s" (include "heimdall.appVersion" $root)) }}
    {{- else }}
      {{- $tags = append $tags . }}
    {{- end }}
  {{- end }}
...
*/}}
{{- define "heimdall.datadogEnvVars" -}}
...
- name: DD_VERSION
  value: {{ include "heimdall.appVersion" . | quote }}
...
*/}}
{{- define "heimdall.datadogContainerName" -}}
