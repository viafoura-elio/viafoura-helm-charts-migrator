...
1. **Check ServiceMonitor Discovery**:
   ...
   kubectl describe servicemonitor heimdall -n monitoring
   ```

2. **Verify Prometheus Targets**:
   ```bash
   kubectl port-forward -n monitoring svc/kube-prometheus-stack-prometheus 9090:9090
   # Open http://localhost:9090/targets and look for heimdall
   ```
...
4. **Test HPA**:
   ```bash
   kubectl describe hpa heimdall -n your-namespace
   kubectl get hpa heimdall -n your-namespace -o yaml
   ```
...
3. **Check ConfigMap and Secret Resources**:
   ```bash
   kubectl get configmap heimdall -n your-namespace -o yaml
   kubectl get secret heimdall -n your-namespace -o yaml
   ```
...
2. **Verify EnvVar Resources**:
   ```bash
   kubectl get configmap heimdall-configMapEnvVars -n your-namespace -o yaml
   kubectl get secret heimdall-secretsEnvVars -n your-namespace -o yaml
   ```
...
1. **Check Pod Status**:
   ...
   kubectl describe pod heimdall-xxx -n your-namespace
   ```

2. **Check Application Logs**:
   ```bash
   kubectl logs -n your-namespace heimdall-xxx --previous
   kubectl logs -n your-namespace heimdall-xxx -c heimdall
   ```
...
1. **Check Rollout Status**:
   ```bash
   kubectl get rollout heimdall -n your-namespace
   kubectl describe rollout heimdall -n your-namespace
   ```
...
3. **Verify Istio Configuration**:
   ```bash
   kubectl get virtualservice heimdall -n your-namespace -o yaml
   kubectl get destinationrule heimdall -n your-namespace -o yaml
   ```
...
2. **Verify Metrics Endpoint**:
   ```bash
   kubectl port-forward -n your-namespace svc/heimdall-metrics 5555:5555
...
1. **Check Service Configuration**:
   ```bash
   kubectl get svc heimdall -n your-namespace -o yaml
   kubectl get endpoints heimdall -n your-namespace
   ```

2. **Test Connectivity**:
   ```bash
   kubectl exec -n your-namespace pod-name -- curl http://heimdall:8080/health
   ```
...
helm get values heimdall -n your-namespace
```
...
kubectl logs -n your-namespace heimdall-xxx -c heimdall --previous

# Follow logs
kubectl logs -n your-namespace heimdall-xxx -c heimdall -f
```
