...
metadata:
  annotations:
    ...
    {{- end }}
    {{- if and .Values.datadog.enabled .Values.datadog.v1.enabled (not .Values.datadog.v2.enabled ) }}
    {{- range $key, $value := .Values.datadog.v1.configs }}
    {{- if $value }}
    ad.datadoghq.com/{{ include "base-chart.datadogContainerName" $ }}.{{ $key }}: |
      {{- $value | toPrettyJson | nindent 6 }}
    {{- end }}
    {{- end }}
    {{- end }}
    {{- if and .Values.datadog.enabled .Values.datadog.v2.enabled (not .Values.datadog.v1.enabled ) }}
    {{- include "base-chart.datadogAnnotations" . | nindent 4 }}
    {{- end }}
    {{- if .Values.istio.enabled }}
    {{- $sidecarContent := include "base-chart.sidecarAnnotations" . | trim }}
    {{- if $sidecarContent }}
    {{- $sidecarContent | nindent 4 }}
    {{- end }}
    {{- end }}
