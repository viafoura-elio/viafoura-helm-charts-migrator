...
func (m *Migrator) copyBaseChart(serviceName string, serviceConfig *config.Service) error {
	...
	capitalizedName := serviceName
	if serviceConfig != nil && serviceConfig.Capitalized != "" {
		capitalizedName = serviceConfig.Capitalized
	} else {
		caser := cases.Title(language.English)
		capitalizedName = caser.String(serviceName)
	}
	...
	return m.chartCopier.CopyBaseChart(src, dst, serviceName, capitalizedName)
}
...
// Priority: 1. Environment containing default_namespace, 2. First enabled environment
func (m *Migrator) determineDefaultEnvironment(cluster config.Cluster, firstEnabledEnv string) string {
	// First, check if default_namespace exists in any environment
	for envName, env := range cluster.Environments {
		if env.Enabled {
			for nsName := range env.Namespaces {
	...
	// If no environment contains default_namespace, use first enabled environment
	if firstEnabledEnv != "" {
		return firstEnabledEnv
	}

	// Fallback to "production" if no environments are enabled (shouldn't happen)
	return "production"
}
...
func (m *Migrator) getEnabledClusters() ([]ClusterInfo, error) {
	...
	if m.config.Clusters == nil || len(m.config.Clusters) == 0 {
	...
	for name, cluster := range m.config.Clusters {
		...
		// Add namespaces from all environments and determine default environment
		var firstEnabledEnv string
		for envName, env := range cluster.Environments {
			if env.Enabled {
				// Track first enabled environment
				if firstEnabledEnv == "" {
					firstEnabledEnv = envName
				}

				for nsName, ns := range env.Namespaces {
					if ns.Enabled {
